...
const DEFAULT_NO_DREAM_NOTE = "No dream recorded.";
const DEFAULT_DREAM_NOTE = "Dream recorded without note.";

const normalizeNote = (dreamType, note) => {
  if (dreamType === "no_dream") {
    return DEFAULT_NO_DREAM_NOTE;
  }

  return note?.trim().length > 0 ? note : DEFAULT_DREAM_NOTE;
};
...
const Home = () => {
  ...
  const saveRecording = async () => {
    ...
    const savedSession = {
      ...
      result: {
        ...
        note: normalizeNote(finalDreamType, finalNote),
      },
  ...
  const saveEditedNote = async () => {
    ...
    const updatedSession = {
      ...
      result: {
        ...session.result,
        note: normalizeNote(dreamType, editedNote),
        editedAt: new Date().toISOString(),
      },
    };

    await saveNight(updatedSession);

    const storedNights = await loadNights();
    const nextSessions = sortSessions(storedNights);

    const updatedIndex = nextSessions.findIndex(
      (item) => item.id === updatedSession.id
    );

    setSleepSessions(nextSessions);
    setCurrentIndex(updatedIndex >= 0 ? updatedIndex : nextSessions.length - 1);
    setIsEditOpen(false);
  };
        ...
        <div className="new-night-card">
          ...
          {modalStep === "recording" && (
            <>
              ...
              <div className="new-night-actions">
                <button className="display-button">Stop</button>
  ...
  const renderEditModal = () => {
    ...
    return (
      <div className="new-night-modal">
        <div className="new-night-card edit-note-card">
          <h2 className="new-night-title">Edit note</h2>

          <textarea
            className="new-night-textarea edit-note-textarea"
            value={editedNote}
            onChange={(event) => setEditedNote(event.target.value)}
            placeholder="Edit your dream note..."
          />

          <div className="new-night-actions edit-buttons-modal">
            <button
              className="new-night-button"
  ...
  const renderNightSlide = () => {
      ...
      return (
        <div className="empty-night-main">
          ...
          <p className="no-new-night-description">
            Press <span className="span-description">New</span> to start your
            first recording.
          </p>
        </div>
      );
    ...
    const note = normalizeNote(dreamType, session.result?.note);
