      ...
      <div className="home-main">
        <div className="swipe-container-main">
          <div className="empty-night-main">
            <h1 className="no-night-title-main">
              No nights yet
            </h1>
          </div>
        </div>
    ...
    <div className="home">
      <div className="swipe-container">
        {sleepSessions.map((session) => {
          ...
          const note = session.result?.note?.trim();
          ...
          return (
            <section className="night-slide" key={session.id}>
              <div className="night-header">
                ...
                <div className="title-dream">
                  <span
                    className={`
                      night-badge
                      ${hasDreamHerbs ? "herbs-active" : "herbs-inactive"}
                    `}
                  >
              ...
              <div className="chart-card">
                <svg viewBox={`0 0 ${width} ${height}`} className="chart">
                  {hourMarks.map((mark) => {
                    const x =
                      ((mark.time - gridStart) / gridDuration) * width;
              ...
              {note && (
                <div className="night-note-wrapper">
                  ...
                  >
                    {openedNotes[session.id] ? "Close note" : "Open note"}
                  </button>

                  {openedNotes[session.id] && (
                    <div className="dream-note-box">{note}</div>
                  )}
                </div>
              )}
            </section>
